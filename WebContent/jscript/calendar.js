...
hideSelectTags = [];function getRealYear(dateObj)
{
...
}function createCalender(year, month, day)
{
	...
	var curYear = getRealYear(curDate)	 // if a date already exists, we calculate some values here
	if (!year)
	{
		var year = curYear;
		var month = curMonth;
	...
	document.getElementById('selectMonth').selectedIndex = month;	 // first day of the month.
	var fristDayOfMonthObj = new Date(year, month, 1);
	var firstDayOfMonth = fristDayOfMonthObj.getDay();	continu		= true;
	firstRow	= true;
	var x	= 0;
	var d	= 0;
	var trs = []
...
}function showCalender(elPos, tgtEl)
{
	...
	var calTable = document.getElementById('calenderTable');	var positions = [0,0];
	var positions = getParentOffset(elPos, positions);	
	calTable.style.left = positions[0]+'px';		
	calTable.style.top = positions[1]+'px';				calTable.style.display='block';	var matchDate = new RegExp('^([0-9]{4})-([0-9]{2})-([0-9]{2})$');
...
function hideSelect(el, superTotal)
{
	...
	for (var c=0; c<totalChilds; c++)
	{
		...
		var calBottom	= calenderEl.offsetTop + calenderEl.offsetHeight;		if (
		(
